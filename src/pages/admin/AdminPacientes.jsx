...
const AdminPacientes = () => {
  ...
  const cancelarEliminarPaciente = () => {
    setEliminarId(null);
  };
  ...
  const handleEliminarPaciente = async () => {
    ...
    setCargando(true);
    try {
      const { error } = await supabase.from('pacientes').delete().eq('id', eliminarId);
      if (error) throw error;
      const { data } = await supabase.from('pacientes').select('*').order('creado_en', { ascending: false });
      if (data) setPacientes(data);
      setEliminarId(null);
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 text-gray-500 text-sm border-b border-gray-100">
                ...
                <th className="px-6 py-4 font-medium text-center w-14">Acción</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {pacientesFiltrados.length === 0 ? (
                <tr>
                  <td colSpan="6" className="px-6 py-8 text-center text-gray-500">No se encontraron pacientes.</td>
                </tr>
              ) : (
                pacientesFiltrados.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-3 py-4 text-center">
                      <button
                        onClick={() => confirmarEliminarPaciente(p.id)}
      ...
      {eliminarId && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-xl max-w-sm w-full p-6 space-y-4">
            <div className="flex justify-between items-center border-b pb-4">
              <h3 className="text-lg font-bold text-red-600">Eliminar paciente</h3>
              <button onClick={cancelarEliminarPaciente} className="text-gray-400 hover:text-gray-600">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>
            {(() => {
              const paciente = pacientes.find(p => p.id === eliminarId);
              return paciente ? (
                <p className="text-gray-600 text-sm">
                  ¿Eliminar a <strong>{paciente.nombre_nino}</strong>?<br></br>
                  Esta acción eliminará <strong>todos sus datos</strong> (historia clínica, citas, financieras).<br></br>
                  No se puede deshacer.
                </p>
              ) : null;
            })()}
            <div className="flex gap-3 pt-2">
              <button onClick={cancelarEliminarPaciente} className="w-1/2 py-2.5 bg-gray-100 text-gray-600 rounded-xl font-medium text-sm hover:bg-gray-200">Cancelar</button>
              <button onClick={handleEliminarPaciente} disabled={cargando} className="w-1/2 py-2.5 bg-red-600 text-white rounded-xl font-medium text-sm hover:bg-red-700 disabled:opacity-50">
